...
function SessionsPageInner() {
  ...
  }, [allSessions, search, selectedSport, selectedCity, selectedDay, selectedLevel, selectedType]);
      ...
      <div>
        ...
        <div className="space-y-1">
          {["All Cities", ...cities].map((c) => {
            ...
            return (
              <button key={c} onClick={() => setSelectedCity(val)}
                className={`w-full text-left px-3 py-2 rounded-lg text-sm font-medium transition-colors ${selectedCity === val ? "text-white" : "text-foreground hover:bg-muted"}`}
      ...
      <div className="border-b bg-secondary/20">
        <div className="container py-8 md:py-12">
          ...
          <p className="text-muted-foreground mb-6">
            {loading ? "Loading sessions…" : `${filtered.length} session${filtered.length === 1 ? "" : "s"} available near you`}
          </p>
